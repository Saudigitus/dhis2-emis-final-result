...
const SOCIO_ECONOMIC_EVENTS = {
  events: {
    ...
    params: ({
      ...
      ouMode: "SELECTED",
      fields:
        "event,enrollment,dataValues[dataElement,value],program,programStage"
    })
  }
}
      ...
      await engine
        .query(REGISTRATION_EVENTS, {
          variables: {
            ...
            academicYearDataElement:
              getDataStoreData?.registration?.academicYear,
            selectedAcademicYear: fieldsWitValue[0].filter(
              (field: any) =>
                field?.id === getDataStoreData?.registration?.academicYear
            )?.[0]?.assignedValue
          }
        })
