...
function ModalContentPromotion(props: ContentProps): React.ReactElement {
  ...
  function onSubmit() {
    ...
    if (
      allFields.filter(
        (element: any) =>
          element?.assignedValue === undefined && element.required
      ).length === 0
    ) {
      // FILTER ALL SELECTED ROWS TEI
      const trackedEntities = selected.rows
        .map((row: any) => row.tei)
        .filter((tei) => {
          return selected.selectedRows.some(
            (event) => tei?.trackedEntity === event?.trackedEntity
          )
        })
      ...
      let listaSemDuplicados = selected.rows
        .map((row: any) => row.tei)
        .filter((tei) => {
          return selected.selectedRows.some(
            (event) => tei?.trackedEntity === event?.trackedEntity
  ...
  const modalActions = [
    ...
    },
    {
      ...
      onClick: () => {
        setClickedButton("saveandcontinue")
        setClicked(true)
      }
    }
  ]
  ...
  if (mutateState.error) {
    show({
      message: "An unknown error occurred while promoting the student",
      type: { critical: true }
    })
  }
  ...
  function onChange(e: any): void {
    ...
    for (const [key, value] of Object.entries(e)) {
      for (let i = 0; i < sections.length; i++) {
        if (
          sections[i].find((element: any) => element.id === key) !== null &&
          sections[i].find((element: any) => element.id === key) !== undefined
        ) {
