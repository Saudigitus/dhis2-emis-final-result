        ...
        valueType:
          programStageDataElement.dataElement?.optionSet != null
            ...
            : (programStageDataElement.dataElement
                ?.valueType as unknown as CustomAttributeProps["valueType"]),
        options: {
          optionSet: programStageDataElement.dataElement?.optionSet ?? undefined
        },
        initialOptions: {
          optionSet: programStageDataElement.dataElement?.optionSet ?? undefined
        },
...
export function formatResponseHeader(
  event: ProgramConfig,
  getDataStoreData: dataStoreRecord
) {
  const dataElements: CustomAttributeProps[] = [];
  const originalData =
    event?.programStages?.find(
      (programStge) =>
        programStge.id === getDataStoreData?.registration?.programStage
